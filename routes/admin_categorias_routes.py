...
async def index(request: Request, usuario_logado: Optional[dict] = None):
    ...
    return RedirectResponse(
        "/admin/categorias/listar", status_code=status.HTTP_307_TEMPORARY_REDIRECT
    )
...
async def listar(request: Request, usuario_logado: Optional[dict] = None):
    ...
    return templates.TemplateResponse(
        "admin/categorias/listar.html", {"request": request, "categorias": categorias}
    )
...
async def get_cadastrar(request: Request, usuario_logado: Optional[dict] = None):
    ...
    return templates.TemplateResponse(
        "admin/categorias/cadastro.html", {"request": request}
    )
...
async def post_cadastrar(
    ...
    if not admin_categorias_limiter.verificar(ip):
        ...
        return RedirectResponse(
            "/admin/categorias/listar", status_code=status.HTTP_303_SEE_OTHER
        )
    ...
    try:
        ...
        logger.info(
            f"Categoria '{dto.nome}' cadastrada por admin {usuario_logado['id']}"
        )
...
async def get_editar(request: Request, id: int, usuario_logado: Optional[dict] = None):
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "categoria": categoria, "dados": dados_categoria},
    )
...
async def post_editar(
    ...
    try:
        ...
        if categoria_existente and categoria_existente.id != id:
            informar_erro(request, "Já existe outra categoria com este nome.")
            return templates.TemplateResponse(
                "admin/categorias/editar.html",
                {
                    "request": request,
        ...
        categoria_atualizada = Categoria(
            id=id, nome=dto.nome, descricao=dto.descricao
        )
...
async def post_excluir(
    ...
    # TODO: Verificar se existem artigos vinculados a esta categoria
    # Quando a funcionalidade de artigos for implementada, adicionar validação aqui

    categoria_repo.excluir(id)
    logger.info(
        f"Categoria {id} ('{categoria.nome}') excluída por admin {usuario_logado['id']}"
    )
